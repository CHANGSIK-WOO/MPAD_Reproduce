...
class schedulerk(torch.nn.Module):
    ...
    def set_timesteps(self, num_inference_steps=100, tradoff=0.5, device='cuda', last_step=-10):
...
class HPAS(torch.nn.Module):
    def __init__(self):
        """

        Harmonic Prompt Aggregation Scheduler (HPAS) of MPAD

        """
        super().__init__()

    def set_timesteps(self, num_inference_steps=100, tradoff=0.5, device='cuda'):
        self.current_step = 0
        self.tradoff = tradoff
...
def prepare_mask_and_masked_image(image, mask, height, width, return_image: bool = False):
    ...
    if isinstance(image, torch.Tensor):
        ...
        if image.min() < -1 or image.max() > 1:
            print("Image should be in [-1, 1] range", image.min(), image.max() )

        # Check mask is in [0, 1]
        if mask.min() < 0 or mask.max() > 1:
            print("Mask should be in [0, 1] range", mask.min(), mask.max() )
...
class StableDiffusionInpaintPipeline(
    ...
    # visual_prompt: Union[str, List[str]] = None,
    @torch.no_grad()
    def __call__(
        self,
        ...
        # print('visual_prompt.shape',visual_prompt.shape)
        ...
        # add scheduler for text embedding 
        self.scheduler_mixup.set_timesteps(num_inference_steps=num_inference_steps, tradoff=tradoff, device=device)
        ...
        for i, t in enumerate(timesteps):
            ...
            prompt_embeds = prompt_embeds * momemtum  + (mixup_alpha*prompt_embedsA +
                                     (1-mixup_alpha)*prompt_embedsB) * (1- momemtum)
            ...
            if return_seq and i % callback_steps == 0:
                seq.append(latents)
        ...
        if not output_type == "latent":
            ...
            if return_seq:
                for latten in seq:
                    images.append(func(latten)[0])
            
            
            image, has_nsfw_concept = func (latents)
            images.append(image)
        ...
        return [StableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept) for image in images]
