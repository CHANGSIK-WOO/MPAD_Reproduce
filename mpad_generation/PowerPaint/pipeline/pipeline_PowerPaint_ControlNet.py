...
def get_mix_alpha(tradoff=0.5, device='cuda', dim=512, prompt_embedsA=None):
        # print(prompt_embedsA.shape)
        # shape = prompt_embedsA.shape
        # for i in shape[1]
        a = torch.zeros(prompt_embedsA.shape[0], device=device, dtype=torch.float16)
        index = torch.argsort(torch.abs(prompt_embedsA), descending=True)
        # print(index)
        # print("prompt_embedsA.shape", prompt_embedsA.shape)
        a[index.view(-1)[:int(tradoff*dim)]]=1
        
        
        return a

class schedulerK(torch.nn.Module):
    def __init__(self,last_step=10,device='cuda'):
        """Custom scheduler of Khoa
        """
        super().__init__()
    ...
    def set_timesteps(self, num_inference_steps=100, tradoff=0.5, device='cuda', dim=512, prompt_embedsA=None):
        self.tradoff = tradoff
        self.num_steps = num_inference_steps-self.last_step
        self.device = self.device
        self.steps = torch.linspace(
            start = tradoff,
            end=0,
            steps=num_inference_steps-self.last_step,
            device=device,
        )
        # alpha=0.7
        
        # a = torch.zeros(dim, device=device, dtype=torch.float16)
        # random inject
        # a[torch.randperm(dim)[:int(tradoff*dim)]] = 1
        # print(tradoff, dim)
        # print(a)
        # print(a.sum())
        # self.alpha=a
    ...
    def step(self):
        # alpha = self.current_step/self.num_steps + self.tradoff        
        self.current_step +=1
        current_step = self.current_step if self.current_step < self.num_steps else self.num_steps-1
        alpha = self.steps[current_step]
        return alpha
...
class StableDiffusionControlNetInpaintPipeline(
    ...
    def __init__(
        ...
        self.scheduler_mixup = schedulerK(last_step=-10)
    ...
    def _encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            prompt_embedsB = prompt_embedsB[0]
            # print("prompt_embedsA.shape", prompt_embedsA.shape)
            # print("prompt_embedsB.shape", prompt_embedsB.shape)
            prompt_embeds = prompt_embedsA*(t)+(1-t)*prompt_embedsB
            # prompt_embeds = []
            # for i in range(prompt_embedsA.shape[1]):
            #     shape = prompt_embedsA[:, i].shape
            #     # print('sub shape', shape)
            #     alpha = get_mix_alpha(tradoff=t, device='cuda', dim=prompt_embedsA.shape[0], prompt_embedsA=prompt_embedsA[:, i].view(-1))
            #     temp = prompt_embedsA[:,i].view(-1)*(alpha)+(1-alpha)*prompt_embedsB[:, i].view(-1)
            #     temp = temp.view(shape)
            #     prompt_embeds.append(temp[:, None, :])
        ...
        prompt_embeds = prompt_embeds.repeat(1, num_images_per_prompt, 1)
        prompt_embeds = prompt_embeds.view(bs_embed * num_images_per_prompt, seq_len, -1)
        prompt_embedsA = prompt_embedsA.repeat(1, num_images_per_prompt, 1)
        prompt_embedsA = prompt_embedsA.view(bs_embed * num_images_per_prompt, seq_len, -1)
        
        prompt_embedsB = prompt_embedsB.repeat(1, num_images_per_prompt, 1)
        prompt_embedsB = prompt_embedsB.view(bs_embed * num_images_per_prompt, seq_len, -1)
        # get unconditional embeddings for classifier free guidance
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            uncond_tokensA: List[str]
            uncond_tokensB: List[str]
        ...
        if do_classifier_free_guidance:
            ...
            # print("prompt_embeds: ",prompt_embeds)
            prompt_embeds = torch.cat([negative_prompt_embeds, prompt_embeds])
            prompt_embedsA = torch.cat([negative_prompt_embeds, prompt_embedsA])
            prompt_embedsB = torch.cat([negative_prompt_embeds, prompt_embedsB])

        return prompt_embeds, prompt_embedsA, prompt_embedsB
    ...
    def __call__(
        ...
        prompt_embeds, prompt_embedsA, prompt_embedsB = self._encode_prompt(
            promptA,
            promptB,
            tradoff,
            device,
        ...
        self.scheduler_mixup.set_timesteps(num_inference_steps=num_inference_steps, tradoff=tradoff, device=device, dim=prompt_embeds.shape[-1], prompt_embedsA=prompt_embedsA)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # print(down_block_res_samples, mid_block_res_sample)
                ...
                if num_channels_unet == 9:
                    latent_model_input = torch.cat([latent_model_input, mask, masked_image_latents], dim=1)
                mixup_alpha = self.scheduler_mixup.step()
                
                # prompt_embeds = prompt_embeds*momemtum  + (mixup_alpha*prompt_embedsB + (1-mixup_alpha)*prompt_embedsA) * (1- momemtum)
                prompt_embeds = prompt_embeds*momemtum  + (mixup_alpha*prompt_embedsA + (1-mixup_alpha)*prompt_embedsB) * (1- momemtum)
